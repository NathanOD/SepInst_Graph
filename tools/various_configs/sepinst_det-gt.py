...
class SepInst(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            inf_size = (batched_inputs[0]["height"], batched_inputs[0]["width"])
            inf_boxes = [x["bbox"] for x in batched_inputs[0]["annotations"]]
            inf_classes = [x["category_id"] for x in batched_inputs[0]["annotations"]]
            inf_scores = [1.0 for _ in inf_classes]
            return self.inference_step(features, features_depth, batched_inputs, images, inf_size, inf_boxes, inf_classes, inf_scores)
    ...
    def inference_step(self, features, features_depth, batched_inputs, images, size, boxes, classes, scores):
        # Proposal Generator
        #rpn_proposals = self.proposal_generator(images, features)
        # Modify proposals to fit the format of Detectron2 training Instances
        gt_proposals = []
        gt_proposal = Instances(size)
        boxes = Boxes(torch.tensor(boxes).to(self.device))
        # convert (x, y, w, h) to (x1, y1, x2, y2) with BoxMode
        boxes.tensor = BoxMode.convert(boxes.tensor, BoxMode.XYWH_ABS, BoxMode.XYXY_ABS)
        gt_proposal.pred_boxes = boxes
        gt_proposal.pred_classes = torch.tensor(classes).to(self.device)
        gt_proposal.scores = torch.tensor(scores).to(self.device)
        gt_proposals.append(gt_proposal)
        # Mask Heads
        instances = self.forward_mask(features, features_depth, gt_proposals)
