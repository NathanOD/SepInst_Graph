...
DEPTH_MASK_HEAD_REGISTRY = Registry("Depth Mask Head")
DEPTH_MASK_HEAD_REGISTRY.__doc__ = "Registry for Depth Mask Heads"
...
@DEPTH_MASK_HEAD_REGISTRY.register()
class ClassicMaskRCNNConvUpsampleHead(BaseMaskRCNNHead, nn.Sequential):
    ...
    def from_config(cls, cfg, input_shape):
        ...
        conv_dim = cfg.MODEL.ROI_RGBD_MASK_HEAD.CONV_DIM
        num_conv = cfg.MODEL.ROI_RGBD_MASK_HEAD.NUM_CONV
        ret.update(
            conv_dims=[conv_dim] * (num_conv + 1),  # +1 for ConvTranspose
            conv_norm=cfg.MODEL.ROI_RGBD_MASK_HEAD.NORM,
            input_shape=input_shape,
        )
        if cfg.MODEL.ROI_RGBD_MASK_HEAD.CLS_AGNOSTIC_MASK:
            ret["num_classes"] = 1
        else:
...
class GCNUpsampleHead(nn.Module):
    def __init__(self, cfg, input_shape):
        ...
        self.channels = cfg.MODEL.ROI_RGBD_MASK_HEAD.IN_CHANNELS
        num_classes = cfg.DATASETS.NUM_CLASSES
        norm = cfg.MODEL.ROI_RGBD_MASK_HEAD.NORM
        num_conv_layers = cfg.MODEL.ROI_RGBD_MASK_HEAD.CONV_LAYERS
        num_gcn_layers = cfg.MODEL.ROI_RGBD_MASK_HEAD.GCN_LAYERS
        num_upsample_layers = cfg.MODEL.ROI_RGBD_MASK_HEAD.UPSAMPLE_LAYERS
        # CNN model
...
def build_depth_mask_head(cfg, input_shape):
    name = cfg.MODEL.ROI_RGBD_MASK_HEAD.NAME
    return DEPTH_MASK_HEAD_REGISTRY.get(name)(cfg, input_shape)