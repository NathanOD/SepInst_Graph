...
class SepInst(nn.Module):
    def __init__(self, cfg):
        ...
        if self.use_depth:
            ...
            backbonedepth_shape = self.backbonedepth.output_shape()
            self.depth_feature_strides = {k: v.stride for k, v in backbonedepth_shape.items()}
            ...
            self.rgbd_mask_head = build_depth_mask_head(
                cfg,
                ShapeSpec(channels=[backbonedepth_shape[f] for f in self.depth_in_features][0].channels,
        ...
        if cfg.WANDB.ENABLED:
            if cfg.MODEL.BACKBONE.NAME == "build_resnet_fpn_backbone":
                backbone_name = f"R{cfg.MODEL.RESNETS.DEPTH}"
            else:
                backbone_name = "Swin"
            self.wandb_run = wandb.init(
                project="SepInst",
                name = f"Graph-{backbone_name}-{cfg.MODEL.HEAD_NAME}-{cfg.DATASETS.TRAIN[0][:-6]}",
                config={
                    ...
                    "clip_gradients": cfg.SOLVER.CLIP_GRADIENTS.ENABLED,
    ...
    def training_step(self, features, features_depth, images, gt_instances):
        ...
        del gt_instances
        # Mask Heads
        mask_losses, _ = self.forward_mask(features, features_depth, rpn_proposals)
