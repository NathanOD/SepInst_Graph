...
from .utils import CustomPredictor
...
class InferenceDemo(object):
    ...
    def run_on_image(self, image, depth, confidence_threshold):
        ...
        for i in range(len(predictions["instances"])):
            mask = predictions["instances"][i].pred_masks
            mask_np = mask.cpu().numpy().astype(np.uint8)
            mask_np = mask_np.squeeze(0)
            rle = mask_util.encode(np.asfortranarray(mask_np))
            rle['counts'] = rle['counts'].decode('utf-8')
            masks.append([rle])
            classes.append(int(predictions["instances"][i].pred_classes.cpu().numpy()))
            scores.append(float(predictions["instances"][i].scores.cpu().numpy()))
